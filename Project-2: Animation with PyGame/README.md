...
- Place your **20‑second animation code** inside the section marked with the comment: *here is the main animation loop*.  
If you like to breakdown the 20-seconds animation into subparts using loops - you are free to do so. (Don't forget to save frames for each of them)  
You can place helper classes and functions anywhere you like. (Make sure to place them before you use them)
...
- Divide the 20 seconds into smaller segments (e.g., 5s rectangles, 5s circle, 5s bubbles, 5s something else).  
...
- [ ] Timed Animation code placed inside the section marked in the template.  
- [ ] Code runs end‑to‑end for the full duration, i.e. **25 seconds** (1+3+1+20 seconds).  
...
- [ ] You should have 1500 frames (25 seconds * 60 frames per second)
- [ ] Open a frame (image) and check the dimensions of the frame to be 1920x1080.
- [ ] Use the command provided in the template to create a video with the generated frames/images.  
  * Command: `ffmpeg -r 60 -start_number {replace with starting sequence no.} -s 1920x1080 -i %d.png -vcodec libx264 -crf 5 -pix_fmt yuv420p final.mp4`  
  (In windows/mac machines, ffmpeg might not come in-built, and will be needed to be installed manually)  
  Your animation video will be created inside the frames directory.  
  * Double check if the video is of 25 seconds and contains all that you wanted to implement.  
  * Delete the video from frames directory.  
- [ ] Backup copy saved before submission.  
- [ ] Create zip of all the correctly numbered images (.png files) generated by running your final animation script insider the directory named *frames*, rename it in correct format (Your_Name.zip) and email to professor following the instructions provided in Canvas (you may keep the TA in CC).
- [ ] Assignment Submission in Canvas:
    * Python code in Text Area (Add a short comment in the beginning explaining about your project).  
